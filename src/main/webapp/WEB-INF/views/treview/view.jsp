  ...
  <div id="wrapper">
    ...
    <section id="maincontent">
      <div class="container">
          ...
          <h3><a href="/treview/list">여행후기 상세</a></h3>
        </div>
        <div class="row">
          <div class="span8">
		            ...
		            <div class="pagination">
		              <ul>
		              	<c:if test="${replyList[0].page ne 1}">
		                	<li><a href="/treview/view?key=${treview.reviewno}&page=${replyList[0].page - 1}">Prev</a></li>
		                </c:if>
		                ...
		                <c:forEach begin="${replyList[0].start_page}" end="${replyList[0].start_page + replyList[0].page_rows - 1}" var="page">
		                	<c:if test="${page <= replyList[0].cnt_page}">
		                		<li class="${page eq replyList[0].page ? 'active' : ''}"><a href="/treview/view?key=${treview.reviewno}&page=${page}">${page}</a></li>
		                	</c:if>
		                </c:forEach>
		                
		                <c:if test="${replyList[0].page ne replyList[0].cnt_page}">
		                	<li><a href="/treview/view?key=${treview.reviewno}&page=${replyList[0].page + 1}">Next</a></li>
		                </c:if>
		              </ul>
		            </div>
		          </div>
